...
A [Gradle](https://gradle.org/) plugin to setup a deobfuscated development environment for Minecraft mods. Primarily used in the Engine toolchain.
...
3. Prepare a project for using the local version of Loom:
   ...
   * Add `mavenLocal()` to the repositories:
     * If you're using `id 'engine-loom'` inside `plugins`, the correct `repositories` block is inside `pluginManagement` in settings.gradle
     * If you're using `apply plugin:` for Loom, the correct `repositories` block is inside `buildscript` in build.gradle
   * Change the loom version to `1.16.local`. For example `id 'engine-loom' version '1.16.local'`
